...
const REMOVE_BOOKMARK_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M11 2C7.229 2 5.343 2 4.172 3.129C3 4.257 3 6.074 3 9.708V17.981C3 20.287 3 21.44 3.773 21.852C5.269 22.652 8.077 19.985 9.41 19.182C10.183 18.717 10.57 18.484 11 18.484C11.43 18.484 11.817 18.717 12.59 19.182C13.923 19.985 16.73 22.652 18.227 21.852C19 21.44 19 20.287 19 17.981V12M21 2L14 9M21 9L14 2" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
...
async function bookmarkActionFunction(article) {
    ...
    if (response.status == 201) {
        ...
        showPopup(2, "You must be logged in to bookmark this article");
    } else if (response.status === 500) {
        showPopup(2, "Internal server error");
    } else {
        showPopup(2, 'Something weird is going on');
    }
}

async function removeBookmarkActionFunction(article) {
    var response = await removeArticleFromBookmarks(article.uuid);
    if (response.status == 201) {
        showPopup(1, "This article has been removed from your bookmarks succesfully");
        // Remove the bookmark from connected users bookmarks ( frontend side )
        userProfile.user.bookmarks = userProfile.user.bookmarks.filter(bookmark => bookmark !== article.uuid);
    } else if (response.status === 400) {
        showPopup(2, "Failed to remove this article from your bookmarks ");
    } else if (response.status === 401 || response.status === 403) {
        showPopup(2, "You must be logged in to remove this article from your bookmarks");
    } else if (response.status === 500) {
        showPopup(2, "Internal server error");
    } else {
        showPopup(2, 'Something weird is going on');
...
export const removeBookmarkAction = {
    svg: REMOVE_BOOKMARK_SVG,
    actionFunction: removeBookmarkActionFunction,
}
